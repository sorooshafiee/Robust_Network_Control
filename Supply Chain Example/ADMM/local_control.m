function [model, b_l, b_u] = local_control(T, P, num_agents, param)
    ...
    Lambda_xi = cell(num_agents, T);
    Lambda_s = cell(num_agents, T);
    ...
    for i = 2 : num_agents
        alpha{i} = model.decision(P, T);
        beta{i} = model.decision(P, T);
    end
    ...
    for t = 1 : T
        ...
        for i = 1 : num_agents
            ...
            Lambda_xi{i,t} = model.decision(P, size(xi_t_{i}, 1));
            lambda{i,t} = model.decision(P, 1);
            if i < num_agents
                s_t_{i+1} = vec(s{i+1}(:, 1:t));
                Lambda_s{i+1,t} = model.decision(P, size(s_t_{i+1}, 1));
                o_t{i} = Lambda_xi{i,t} * xi_t_{i} + Lambda_s{i+1,t} * s_t_{i+1} + lambda{i,t};
            else
                o_t{i} = Lambda_xi{i,t} * xi_t_{i} + lambda{i,t};
            end
        end
    ...
    model.solve()
    b_u = cell(num_agents, 1);
    b_l = cell(num_agents, 1);
    for i = 2 : num_agents
        b_u{i} = beta{i}.get + alpha{i}.get;
        b_l{i} = beta{i}.get - alpha{i}.get;
    end
end